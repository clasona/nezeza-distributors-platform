return (
  <div>
    ...
    <div className='w-full p-6 grid grid-cols-1 md:grid-cols-2 gap-6'>
      {' '}
      {/* Removed container and added w-full*/}
      {/* Product Images */}
      ...
      <div>
        ...
        <div className='mt-6 flex flex-col sm:flex-row gap-4'>
          <Button className='bg-nezeza_dark_blue text-white flex items-center px-4 sm:px-6 py-2 text-sm sm:text-base rounded-lg hover:bg-blue-700 transition w-full sm:w-auto'>
            <ShoppingCart className='mr-2' /> Add to Cart
          </Button>
          <Button className='bg-nezeza_green_600 text-white px-4 sm:px-6 py-2 text-sm sm:text-base rounded-lg hover:bg-green-700 transition w-full sm:w-auto'>
            Buy Now
          </Button>
          <Button className='border border-gray-300 flex items-center px-4 sm:px-6 py-2 text-sm sm:text-base rounded-lg hover:bg-gray-100 transition w-full sm:w-auto'>
        ...
        <div className='flex items-center mt-4'>
          {[...Array(5)].map((_, i) => (
            <Star
              key={i}
              className={`h-5 w-5 ${
                i < product.averageRating ? 'text-yellow-500' : 'text-gray-300'
              }`}
            />
          ))}
