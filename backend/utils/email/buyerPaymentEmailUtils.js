...
const sendBuyerPaymentConfirmationEmail = async ({ name, email, orderId }) => {
  ...
  // Use the actual order data instead of recalculating
  const totalAmount = order.totalAmount || 0;
  const totalTax = order.totalTax || 0;
  const totalShipping = order.totalShipping || 0;
  const processingFee = order.processingFee || 0;
  
  // Calculate product subtotal from actual order items
  const productSubtotal = order.orderItems.reduce((sum, item) => {
    ...
    <div style="background-color: #f0f9ff; padding: 20px; border-radius: 8px; margin: 20px 0;">
      ...
      <table style="width: 100%; border-collapse: collapse;">
        ...
        <tr>
          ...
          <td style="padding: 8px 0; border-bottom: 1px solid #e5e7eb; text-align: right;">$${totalTax.toFixed(2)}</td>
        </tr>
        <tr>
          <td style="padding: 8px 0; border-bottom: 1px solid #e5e7eb;"><strong>Shipping:</strong></td>
          <td style="padding: 8px 0; border-bottom: 1px solid #e5e7eb; text-align: right;">$${totalShipping.toFixed(2)}</td>
        </tr>
        ${processingFee > 0 ? `
        <tr>
          <td style="padding: 8px 0; border-bottom: 1px solid #e5e7eb;"><strong>Processing Fee:</strong></td>
          <td style="padding: 8px 0; border-bottom: 1px solid #e5e7eb; text-align: right;">$${processingFee.toFixed(2)}</td>
        </tr>` : ''}
        <tr style="background-color: #dbeafe;">
          <td style="padding: 12px 8px; font-size: 18px;"><strong>Total Paid:</strong></td>
          <td style="padding: 12px 8px; text-align: right; font-size: 18px;"><strong>$${totalAmount.toFixed(2)}</strong></td>
        </tr>
      </table>
      ${processingFee > 0 ? `
