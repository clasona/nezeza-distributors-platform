...
const getAllUsers = async (req, res) => {
    ...
    const allUsers = await User.find({_id: { $ne: req.user.userId }}).select('-password').populate('roles');
    if (!allUsers || allUsers.length === 0) {
      throw new CustomError.NotFoundError('No users found on the platform at the moment');
    }
