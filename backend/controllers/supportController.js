...
const createSupportTicket = async (req, res) => {
  try {
    ...
    // Handle attachments as simple URL array
    let attachmentUrls = [];
    if (attachments) {
      try {
        // Parse JSON string if it's a string, otherwise use directly
        const attachmentData = typeof attachments === 'string' 
          ? JSON.parse(attachments)
          : attachments;
        
        // Validate that it's an array of strings (URLs)
        if (Array.isArray(attachmentData)) {
          attachmentUrls = attachmentData.filter(url => 
            typeof url === 'string' && 
            url.length > 0 &&
        ...
        console.warn('Failed to parse attachments:', parseError.message);
        attachmentUrls = [];
      }
    }

    // Handle traditional file uploads as fallback (deprecated but kept for compatibility)
    if (req.files && req.files.attachments && attachmentUrls.length === 0) {
      ...
      for (const file of files) {
        ...
        const fileUrl = `/uploads/support/${uniqueFilename}`;
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[CREATE_TICKET] Processing ${attachmentUrls.length} attachments for user ${userId}`);
    }
...
const addMessageToTicket = async (req, res) => {
  try {
    ...
    if (attachments) {
      try {
        ...
        if (Array.isArray(attachmentData)) {
          ...
          // Limit to 5 attachments for replies
    ...
    // Log attachment processing for debugging
    if (process.env.NODE_ENV === 'development') {
      console.log(`[ADD_MESSAGE] Processing ${attachmentUrls.length} attachments for ticket ${ticketId}`);
    }
