...
} = require('../utils/cloudinaryAttachmentUtils');
...
const createSupportTicket = async (req, res) => {
  try {
    ...
    const {
      ...
      subOrderId,
      productId,
      cloudinaryAttachments, // New field for Cloudinary attachments
    } = req.body;
    
    // Handle traditional file attachments
    let traditionalAttachments = [];
    if (req.files && req.files.attachments) {
      ...
      for (const file of files) {
        ...
        traditionalAttachments.push(attachment);
    ...
    let parsedCloudinaryAttachments = [];
    if (cloudinaryAttachments) {
      try {
        // Parse JSON string if it's a string, otherwise use directly
        const cloudinaryData = typeof cloudinaryAttachments === 'string' 
          ? JSON.parse(cloudinaryAttachments)
          : cloudinaryAttachments;
        
        // Only validate if there are actual attachments
        if (Array.isArray(cloudinaryData) && cloudinaryData.length > 0) {
          // Validate Cloudinary attachments
          const validation = validateCloudinaryAttachments(cloudinaryData);
          if (!validation.isValid) {
            throw new CustomError.BadRequestError(
              `Invalid Cloudinary attachments: ${validation.errors.join(', ')}`
            );
          }
        }
        
        parsedCloudinaryAttachments = cloudinaryData;
      } catch (parseError) {
        throw new CustomError.BadRequestError(
          'Invalid Cloudinary attachments format. Must be valid JSON.'
        );
      }
    }

    // Combine all attachments
    const allAttachments = combineAttachments(traditionalAttachments, parsedCloudinaryAttachments);
    ...
    // Only add orderId, subOrderId, productId if they are valid ObjectIds
    if (orderId && orderId.match(/^[0-9a-fA-F]{24}$/)) {
      ticketData.orderId = orderId;
    }
    
    if (subOrderId && subOrderId.match(/^[0-9a-fA-F]{24}$/)) {
      ticketData.subOrderId = subOrderId;
    }
    
    if (productId && productId.match(/^[0-9a-fA-F]{24}$/)) {
      ticketData.productId = productId;
    }
...
const addMessageToTicket = async (req, res) => {
  try {
    const { ticketId } = req.params;
    const userId = req.user.userId;
    const { message, cloudinaryAttachments } = req.body;
    
    // Handle traditional file attachments for replies
    let traditionalAttachments = [];
    if (req.files && req.files.attachments) {
      const files = Array.isArray(req.files.attachments) ? req.files.attachments : [req.files.attachments];
    ...
    // Handle Cloudinary attachments for replies
    let parsedCloudinaryAttachments = [];
    if (cloudinaryAttachments) {
      try {
        // Parse JSON string if it's a string, otherwise use directly
