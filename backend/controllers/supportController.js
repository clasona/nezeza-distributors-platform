...
const {
  ...
  generateDownloadUrl
} = require('../utils/cloudinaryPdfNormalizer');
  ...
  try {
    ...
    if (req.files && req.files.attachments && cloudinaryUrls.length === 0) {
      ...
      for (const file of files) {
        ...
        const fileUrl = `${req.protocol}://${req.get('host')}/uploads/support/${uniqueFilename}`;
        traditionalFileUrls.push(fileUrl);
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`[CREATE_TICKET] Processing ${allAttachments.length} attachments for user ${userId}`);
    }
...
const addMessageToTicket = async (req, res) => {
  try {
    ...
    // Log attachment processing for debugging
    if (process.env.NODE_ENV === 'development') {
      console.log(`[ADD_MESSAGE] Processing ${allAttachments.length} attachments for ticket ${ticketId}`);
    }
